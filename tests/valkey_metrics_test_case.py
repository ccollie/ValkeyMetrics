...
class ValkeyMetricsTestCaseBase(ValkeyTestCase):
    ...
    def create_time_series_and_add_items(self, client, number_of_bf=5):
        ...
        for i in range(number_of_bf):
            assert client.execute_command(f'VM.ADD SAMPLE{i} FOO') == 1, f"Failed to insert bloom filter item SAMPLE{i} FOO"
    ...
    def validate_copied_series_correctness(self, client, original_series_name, item_prefix, add_operation_idx, expected_fp_rate, fp_margin, original_info_dict):
        """ Validate correctness on a copy of the provided time series.
        """
        copy_series_name = "series_copy"
        assert client.execute_command(f'COPY {original_series_name} {copy_series_name}') == 1
        object_digest = client.execute_command(f'DEBUG DIGEST-VALUE {original_series_name}')
        copied_object_digest = client.execute_command(f'DEBUG DIGEST-VALUE {copy_series_name}')
        assert copied_object_digest == object_digest
        assert client.execute_command('DBSIZE') == 2
        copy_info = client.execute_command(f'VM.SERIES-INFO {copy_series_name}')
