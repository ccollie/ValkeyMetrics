use crate::alerts::meta::{
    clear_all_group_managers, clear_group_manager, swap_group_manager_dbs, with_group_manager,
};
...
fn handle_key_rename(ctx: &Context, old_key: &[u8], new_key: &[u8]) {
    let is_ts = with_timeseries_index(ctx, |index| index.rename_series(ctx, old_key, new_key));
    if !is_ts {
        with_group_manager(ctx, |manager| manager.rename_group(old_key, new_key));
    }
}
...
fn remove_key_from_index(ctx: &Context, key: &[u8]) {
    ...
    if !is_ts {
        // see if it's a group
        with_group_manager(ctx, |manager| manager.delete_group_by_key(ctx, key));
    }
}
