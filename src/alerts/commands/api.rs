...
use crate::alerts::rules::{
    AlertingRule, Group, MetricRule, RecordingRule, Rule, RuleStateEntry, RuleType,
};
...
fn rule_state_entry_value(state: &RuleStateEntry) -> ValkeyValue {
    ...
    result.insert(
        "duration".into(),
        ValkeyValue::from(state.duration.as_secs_f64()),
    );
    result.insert("samples".into(), ValkeyValue::from(state.samples));
    result.insert(
        "series_fetched".into(),
        ValkeyValue::from(state.series_fetched),
    );
...
pub(super) fn recording_rule_to_api(group: &Group, rr: &RecordingRule) -> ValkeyValue {
    ...
    let updates = rr
        .get_all_entries()
        .iter()
        .map(rule_state_entry_value)
        .collect();
    hash.insert(
        "id".into(),
        ValkeyValue::BulkString(format!("{}", rr.rule_id)),
    );
    ...
    hash.insert(
        "last_evaluation".into(),
        ValkeyValue::Integer(last_state.time),
    );
    hash.insert(
        "evaluation_time".into(),
        ValkeyValue::Float(last_state.duration.as_secs_f64()),
    );
    hash.insert(
        "last_samples".into(),
        ValkeyValue::Integer(last_state.samples as i64),
    );
    if let Some(last_fetched) = last_state.series_fetched {
        hash.insert(
            "last_series_fetched".into(),
            ValkeyValue::Integer(last_fetched as i64),
        );
    }
    hash.insert(
        "max_updates".into(),
        ValkeyValue::Integer(max_updates as i64),
    );
    hash.insert("updates".into(), ValkeyValue::Array(updates));
    hash.insert(
        "group_id".into(),
        ValkeyValue::BulkString(group.id.to_string()),
    );
    hash.insert(
        "group_name".into(),
        ValkeyValue::BulkString(group.name.clone()),
    );
    ...
    hash.insert(
        "memory_usage".into(),
        ValkeyValue::Integer(rr.get_size() as i64),
    );

    ValkeyValue::Map(hash)
}

pub(super) fn alerting_rule_to_api(
    group: &Group,
    ar: &AlertingRule,
    exclude_alerts: bool,
) -> ValkeyValue {
    ...
    hash.insert(
        "keep_firing_for".into(),
        ValkeyValue::Float(keep_firing_for),
    );
    hash.insert("labels".into(), string_hash_map_to_value(&ar.labels));
    hash.insert(
        "annotations".into(),
        string_hash_map_to_value(&ar.annotations),
    );
    hash.insert("last_evaluation".into(), ValkeyValue::from(last_evaluation));
    hash.insert(
        "evaluation_time".into(),
        ValkeyValue::Float(evaluation_time),
    );
    ...
    hash.insert(
        "memory_usage".into(),
        ValkeyValue::Integer(ar.get_size() as i64),
    );
...
pub(super) fn new_alert_api(ar: &AlertingRule, a: &Alert) -> ValkeyValue {
    ...
    hash.insert(
        "expression".into(),
        ValkeyValue::BulkString(ar.expr.clone()),
    );
    hash.insert("labels".into(), string_hash_map_to_value(&a.labels));
    hash.insert(
        "annotations".into(),
        string_hash_map_to_value(&a.annotations),
    );
    ...
    hash.insert(
        "value".into(),
        ValkeyValue::BulkString(format!("{}", a.value)),
    );
...
pub(super) fn group_to_api(group: &Group, filter: Option<&RulesFilter>) -> ValkeyValue {
    ...
    hash.insert(
        "id".into(),
        ValkeyValue::BulkString(format!("{}", group.id())),
    );
    hash.insert("name".into(), ValkeyValue::BulkString(group.name.clone()));
    hash.insert(
        "interval".into(),
        ValkeyValue::Float(group.interval.as_secs_f64()),
    );
    hash.insert("last_evaluation".into(), ValkeyValue::from(last_evaluation));
    hash.insert("params".into(), string_hash_map_to_value(&group.params));
    hash.insert(
        "notifier_headers".into(),
        string_hash_map_to_value(&group.params),
    );
    ...
    hash.insert(
        "memory_usage".into(),
        ValkeyValue::Integer(group.get_size() as i64),
    );

    ValkeyValue::Map(hash)
}

fn filtered_rules_to_value(
    group: &Group,
    rules: &[MetricRule],
    filter: Option<&RulesFilter>,
) -> ValkeyValue {
    if let Some(filter) = filter {
        ...
        let res = rules
            .iter()
            .map(|r| rule_to_api(group, r, false))
            .collect::<Vec<_>>();
