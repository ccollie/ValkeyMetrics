...
static _ALERT_SETTINGS: LazyLock<Mutex<AlertSettings>> = LazyLock::new(|| Mutex::new(AlertSettings::default()));
static _QUERY_CONTEXT_CONFIG: LazyLock<Mutex<SessionConfig>> = LazyLock::new(|| Mutex::new(SessionConfig::default()));
...
static _SERIES_SETTINGS: LazyLock<Mutex<SeriesSettings>> = LazyLock::new(|| Mutex::new(SeriesSettings::default()));
...
pub static QUERY_ROUND_DIGITS: LazyLock<Option<u8>> = LazyLock::new(|| {
    *_QUERY_ROUND_DIGITS.lock().unwrap()
});

pub static QUERY_DEFAULT_STEP: LazyLock<Duration> = LazyLock::new(|| {
    *_QUERY_DEFAULT_STEP.lock().unwrap()
});
...
fn get_duration_config_value_ms(args: &[ValkeyString], name: &str, default_duration: Option<i64>) -> ValkeyResult<i64> {
...
fn get_duration_config_value(args: &[ValkeyString], name: &str, default_duration: Option<Duration>) -> ValkeyResult<Duration> {
    get_duration_config_value_ms(args, name, default_duration.map(|d| d.as_millis() as i64))
        .map(|ms| Duration::from_millis(ms as u64))
}

fn get_optional_duration_config_value(args: &[ValkeyString], name: &str) -> ValkeyResult<Option<Duration>> {
...
fn get_number_config_value(args: &[ValkeyString], name: &str, default_value: Option<f64>) -> ValkeyResult<f64> {
    if let Some(value) = find_config_value(args, name) {
...
fn get_optional_number_config_value(args: &[ValkeyString], name: &str) -> ValkeyResult<Option<f64>> {
    if find_config_value(args, name).is_some() {
...
fn get_rounding_strategy_digit_value(args: &[ValkeyString], name: &str) -> ValkeyResult<Option<i32>> {
    if let Some(index) = args.iter().position(|arg| arg.as_slice().eq(name.as_bytes())) {
...
fn load_query_context_config(args: &[ValkeyString]) -> ValkeyResult<()> {
    ...
    let mut res = _QUERY_CONTEXT_CONFIG.lock()
        .map_err(|_| ValkeyError::String("mutex lock error setting query context config".to_string()))?;
...
fn load_alert_settings(args: &[ValkeyString]) -> ValkeyResult<()> {
    ...
    config.max_resolve_duration = get_duration_config_value(args, RULES_MAX_RESOLVE_DURATION_KEY, Some(Duration::from_secs(0)))?;
    config.resend_delay = get_duration_config_value(args, RULES_EVAL_DELAY_KEY, Some(DEFAULT_RESEND_DELAY))?;
  //  config.external_labels = get_external_labels_config_value(args)?;
    config.look_back = get_duration_config_value(args, RULES_LOOKBACK_KEY, Some(DEFAULT_ALERT_LOOKBACK))?;
    config.eval_delay = get_duration_config_value(args, RULES_REMOTE_READ_LOOKBACK_KEY, Some(DEFAULT_EVAL_DELAY))?;
    config.query_step = get_duration_config_value(args, RULES_QUERY_STEP_KEY, Some(DEFAULT_STEP))?;
    config.disable_alert_group_labels = get_bool_config_value(args, RULES_DISABLE_ALERT_GROUP_LABEL_KEY, false);
    config.evaluation_interval = get_duration_config_value(args, RULES_GROUP_EVAL_INTERVAL_KEY, Some(DEFAULT_GROUP_EVAL_INTERVAL))?;
    config.rule_update_entries_limit = get_number_config_value(args, RULE_UPDATE_ENTRIES_LIMIT_KEY, Some(DEFAULT_RULE_UPDATE_ENTRIES_LIMIT as f64))? as usize;
    config.replay_rules_delay = get_duration_config_value(args, RULES_REPLAY_RULES_DELAY_KEY, Some(Duration::from_secs(0)))?;
    ...
    if let Some(v) = config.round_digits {
        if v > 18 {
            return Err(ValkeyError::String(format!("Invalid value for {}: {}", RULES_ROUND_DIGITS_KEY, v)));
        }
    }
    
    let mut res = _ALERT_SETTINGS.lock()
...
pub fn load_config(_ctx: &Context, args: &[ValkeyString]) -> ValkeyResult<()> {
    ...
    *prefix = key_prefix.to_string();
    ...
    let round_digits = get_optional_number_config_value(args, QUERY_ROUND_DIGITS_KEY)?.map(|v| v as u8);
    let mut temp = _QUERY_ROUND_DIGITS.lock()
        .map_err(|_| ValkeyError::String("mutex lock error setting query round digits".to_string()))?;
    
    *temp = round_digits;
    
    let default_step = get_duration_config_value(args, QUERY_DEFAULT_STEP_KEY, Some(DEFAULT_STEP))?;
    let mut temp = _QUERY_DEFAULT_STEP.lock()
        .map_err(|_| ValkeyError::String("mutex lock error setting query default step".to_string()))?;
