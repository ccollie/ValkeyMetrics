            ...
            dst_off_bits: usize, src_off_bits: usize) -> Result<(), BitCopyError>
{
    ...
    let copy_size: usize = size_of::<u8>();
    let copy_bit_size: usize = copy_size * 8;
    ...
    while nbits > 0 {
        let src_bit_offset = src_offbits & (copy_bit_size - 1);
        let src_bits = copy_bit_size - src_bit_offset;
        ...
        let dst_bit_offset = dst_offbits & (copy_bit_size - 1);
        let dst_bits = copy_bit_size - dst_bit_offset;
        ...
        if dst_copy_bits <= dst_bits {
            ...
            dst_buf[dst_idx + 1] = (byte & byte_mask) << (copy_bit_size - dst_copy_bits);
        }
...
#[cfg(test)]
mod tests {
    use super::super::varint::{decode, encode};
    use super::*;
