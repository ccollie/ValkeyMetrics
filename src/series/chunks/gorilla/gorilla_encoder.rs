...
impl GorillaEncoder {
    ...
    fn write_second_sample(&mut self, sample: &Sample) -> std::io::Result<()> {
        ...
        let (leading, trailing) =
            write_varbit_xor(value, self.value, 0xff, 0, &mut self.writer)?;
    ...
    pub fn rdb_save(&self, rdb: *mut raw::RedisModuleIO) {
        ...
        raw::save_unsigned(rdb, self.leading_bits as u64);
        raw::save_unsigned(rdb, self.trailing_bits as u64);
    ...
    pub fn rdb_load(rdb: *mut raw::RedisModuleIO) -> Result<GorillaEncoder, ValkeyError> {
        ...
        let leading_bits = raw::load_unsigned(rdb)? as u8;
        let trailing_bits = raw::load_unsigned(rdb)? as u8;
