...
cfg_if! {
    if #[cfg(feature = "id64")] {
        pub(crate) use xxhash_rust::xxh3::Xxh3 as IdHasher;
        pub(crate) use croaring::Bitmap64 as IdBitmap;
    } else {
        pub(crate) use xxhash_rust::xxh32::Xxh32 as IdHasher;
...
impl Postings {
    ...
    pub fn index_time_series(&mut self, ts: &TimeSeries, key: &[u8]) {
    ...
    pub fn reindex_timeseries(&mut self, ts: &TimeSeries, key: &[u8]) {
        self.remove_series_by_id(ts.id, &ts.metric_name, &ts.labels);
        self.index_time_series(ts, key);
    }

    pub fn remove_series(&mut self, ts: &TimeSeries) {
        self.remove_series_by_id(ts.id, &ts.metric_name, &ts.labels);
